          ...
          <div className="flex-shrink-0">
            <Link to="/" onClick={closeMenu} className="flex items-center gap-3 group">
              ...
              <div className="flex flex-col">
                <span className="font-serif text-2xl sm:text-3xl font-bold text-white tracking-tight leading-none group-hover:text-gold-50 transition-colors">
                  MACON
                </span>
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              ...
              <div className="relative group">
                <button className="hover:text-gold-500 transition-colors px-3 py-2 rounded-md font-medium inline-flex items-center text-sm lg:text-base">
                  Service Areas
                </button>
                {/* Dropdown */}
                <div className="absolute left-0 mt-0 w-56 bg-white rounded-md shadow-lg py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform z-50 border-t-2 border-gold-500">
                  {SERVICE_AREAS.map(area => (
                    <Link
                      key={area.id}
                      to={`/area/${area.id}`}
                      className="block px-4 py-3 text-sm text-navy-900 hover:bg-navy-50 hover:text-gold-600 border-b border-gray-50 last:border-0"
                    >
                      {area.name}
                    </Link>
                  ))}
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-navy-800 border-t border-navy-700">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <Link to="/attorneys" onClick={closeMenu} className="block px-3 py-2 rounded-md text-base font-medium text-white hover:text-gold-500">Attorneys</Link>
            <div className="px-3 py-2 text-gold-500 text-xs uppercase font-bold tracking-wider">Areas We Serve</div>
            {SERVICE_AREAS.map(area => (
              <Link
                key={area.id}
                to={`/area/${area.id}`}
